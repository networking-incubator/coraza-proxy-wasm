...
A filter state key is only set when its value is non-empty; fields such as `severity`, `category`, and `client_ip` are omitted when Coraza cannot resolve the matched rule for the interruption.
